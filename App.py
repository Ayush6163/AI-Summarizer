...
# -------------------------
SVG_HEADER = """
<div style="background: linear-gradient(90deg,#0f172a,#0b1221); padding:26px; border-radius:10px;">
  <h1 style="color:white; margin:0; font-family: Inter, sans-serif;">📝 EchoNotes — Smart Summarizer</h1>
  <p style="color:#cbd5e1; margin-top:6px; font-size:14px;">Summarize articles, PDFs, or pasted text. Extract keywords, action items and a word cloud — no paid API required.</p>
</div>
"""
...
st.markdown(
    """
    **Tips:**  
    - For best results, paste a single article, transcript, or 1–3 pages of text.  
    - Use URL mode for news articles (Newspaper3k tries to extract main text).  
    - PDF upload works best with text PDFs (not scanned images).  
    - Toggle algorithms to compare: LexRank (concise), LSA (themes), TextRank (central sentences).
    """
)
st.markdown("Built with ❤️ using Sumy (optional), Newspaper3k (optional), PyPDF2 (optional), WordCloud and Streamlit — no paid API keys required.")
# summarizer.py